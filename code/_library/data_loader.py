...
def loader_nonFinancialReports(folderPath, companies = [], read_text = False):
    ...
    for yearlyFolder in scandir(folderPath):
        ...
        for fileName in files:
            ...
            if len(partialComponents) < 2 or len(partialComponents) > 4:
            ...
            # Normalize companyName for comparison
            companyName = companyName.replace(' ', '_').lower()
            print('companyName.lower():', companyName.lower())
            if len(companies) > 0 and companyName.lower() not in companies:
                print(f"Skipping company {companyName} as it is not in the provided list of companies.")
                continue
...
def load_esg_categories(file_path = path.join('models', 'prompt', 'esg_topics.json')):
    file_path = 'E:\\derivingStructure\\code\\models\\prompt\\esg_topics.json'
    print('Loading ESG categories from', file_path)
    with open(file_path, 'r', encoding='utf-8') as esg_categories:
